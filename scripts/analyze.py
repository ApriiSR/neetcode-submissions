...
SYSTEM_PROMPT = (
    ...
    "removes a machine-word guarantee. That holds specifically when the "
    "numbers themselves grow with the input: decoding an n-digit list into "
    "one integer and adding it is O(n), not O(n^2), because digit-level and "
    "bit-level work is never counted in a reported complexity, however large "
    "the operands get. When that digit work is the real reason one approach "
    "is slower than another it belongs in \"notes\", said in prose — the "
    "reported figure stays unit-cost so two submissions to the same problem "
    "stay comparable. "
